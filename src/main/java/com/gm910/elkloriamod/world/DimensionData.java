...
public class DimensionData extends WorldSavedData {
	...
	public static final String NAME = Elkloria.MODID + "_dimensions";
	...
	public static final ResourceLocation USIFIA_RL = new ResourceLocation(Elkloria.MODID, "usifia");
	public static DimensionType USIFIA = null;
	...
	public static void registerWorldMaker(String name, BiFunction<World, DimensionType, ? extends Dimension> dimensionconstructor) {
		
		DimensionInit.WORLD_MAKERS.register(name, () -> ElkloriaDimensionFactory.withFactory(dimensionconstructor));
	}
	...
	public DimensionType createDimension(ResourceLocation name, RegistryObject<ModDimension> worldmaker, boolean hasSkyLight) {
		if (DimensionInit.from(worldmaker) != null) {
			if (!DIMENSIONS.containsKey(name)) {
				...
				if (createdDim == null) return null;
	...
	public static DimensionType $registerDimension(ResourceLocation rname, BiFunction<World, DimensionType, ? extends Dimension> dimensionconstructor, boolean hasSkyLight) {
		if (DimensionType.byName(rname) == null) {
			DimensionType createdDim = DimensionManager.registerDimension(rname, new ElkloriaDimensionFactory(dimensionconstructor), null, hasSkyLight);
			return createdDim;
		}
		return DimensionType.byName(rname);
	}
	
	public static DimensionType $registerDimension(ResourceLocation rname, RegistryObject<ModDimension> obj, boolean hasSkyLight) {
		if (DimensionType.byName(rname) == null) {
	...
	public static DimensionType registerInitialDimension(ResourceLocation rname, RegistryObject<ModDimension> worldmaker, boolean hasSkyLight) {
	...
	public static DimensionType registerInitialDimension(ResourceLocation rname, String worldmaker, boolean hasSkyLight) {
	...
	/**
	 * This class is a way of storing dimension information for the modular dimension system I'm trying to make...
	 * @author GameCraft910
	 *
	 */
	public static class DimensionInfo {
		...
		public static DimensionInfo fromNBT(CompoundNBT nbt) {
			...
			if (nbt.contains("Name", NBT.TAG_STRING) && nbt.contains("Worldmaker", NBT.TAG_STRING) && nbt.contains("SkyLight")) {
				DimensionInfo inf = new DimensionInfo();
		...
		/**
		 * Registers this dimensionInfo to the forge dimension registry as a proper dimension
		 * If it already is registered, simply returns the existing dimensionType corresponding to it
		 * @param data
		 * @return
		 */
		public DimensionType registerDimensionType(@Nullable PacketBuffer data) {
			if (DimensionType.byName(name) == null) {
				DimensionType createdDim = DimensionManager.registerDimension(name, DimensionInit.from(worldmaker), data, hasSkyLight);
				return createdDim;
			}
